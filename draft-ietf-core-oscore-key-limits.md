...
A limitation that is introduced is that, in order to not exceed the selected value for 'l', the total size of the COSE plaintext {{RFC9052}}, authentication tag, and possible ciphertext padding for a message must not exceed the block size for the selected algorithm multiplied by 'l'. The size of the COSE plaintext is calculated as described in {{Section 5.3 of RFC8613}}.
...
* SSN\* is the current value of the other peer's Sender Sequence Number. That is, SSN\* is an overestimation of the responses without Partial IV that this peer has sent.
...
* If the outgoing message is a request, X is the highest Partial IV value marked as received in this peer's Replay Window plus 1, or 0 if it has not accepted any protected message from the other peer yet. Note that, also in this case, X < SSN\* always holds.
